-- The course tree is rebuilt atomically. Explicit authorization at the RPC
-- boundary allows its internal hierarchy writes to bypass intermediate RLS
-- snapshots without granting the caller direct access to another course.
...
declare
  ...
  v_existing_status public.course_status;
begin
  ...
  if p_course_id is null then
    ...
    into v_existing_author, v_existing_status
    from public.courses
    ...
    if not (select public.is_admin()) and (
      v_existing_author is distinct from v_user_id
      or v_existing_status not in ('draft', 'changes_requested')
    ) then
      raise exception 'Course not found or cannot be edited';
    end if;
